...
@node Muc Shell, Muc Shell Overview, , Top
@chapter Muc Shell
...
@c
@node Muc Shell Overview, Muc Shell Source, Muc Shell, Muc Shell
@section Muc Shell Overview

This chapter documents the in-db (@sc{muc}) implementation of the
@sc{muc} shell, and includes all the source.  You most definitely
do not need to read or understand this chapter in order to use
the @sc{muc}, shell, but you may find it interesting if you
are curious about the internals of the @sc{muc} shell, or are
interested in writing a Muq shell of your own.

@c
@node Muc Shell Source, Muc Shell Wrapup, Muc Shell Overview, Muc Shell
@section Muc Shell Source

@example  @c

( - 212-C-muc-shell.muf -- Shell using "Multi-User C".			)
...
( 	C++ is for people who like to walk tightropes without a net.	)
(	MUC is for people who like to sit in overstuffed armchairs.	)
...
: ]shell { [] -> ? }
    ...
    [   :function :: { -> ! }  'abrt goto ;
	:name 'abort
	:reportFunction "Return to main mucShell prompt."
    | ]withRestartDo{               ( 3 )

    ( Establish a restart letting users   )
    ( exit this shell from the debugger:  )
    [   :function :: { -> ! }  'muc:exitShell goto ;
	:name 'exit
...
( =====================================================================	)
( - shell -- Easy invocation of vanilla muc shell.			)
