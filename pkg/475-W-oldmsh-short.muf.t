...
( - 475-W-oldmsh-short.muf -- Set-short-description for mudUser shell.	)
...
( Created:      97Oct19, from 475-W-oldmsh-ping.t			)
( Modified:								)
( Language:     MUF							)
( Package:      N/A							)
( Status:       							)
...
( defmethod:  cmdHelpCategory { 'cmdShort }	)
(    {[                         'it      ]}	)
(    [ "" |                              	)
( ;                                      	)
...
defmethod:  cmdHelp1 { 'cmdShort }
    {[                   'it       ]}

    [ "Set short description on <object>." |
;
...
defmethod:  cmdHelpN { 'cmdShort }
    {[                   'it       ]}

    [
"Set short description on given object: @short <myname>=<myDescription>"
    |
;
...
defmethod: cmdDo { 'cmdShort 't  't     }
        |shift -> it
        |shift -> av
        |shift -> cmdName
	'=' charInt |position -> pos
	pos not if "Syntax is '@short <myname>=<myDescription>" errcho ]pop [ | return fi
    ...
    [ obj "short" "plain" shortDesc | oldmud:doSetView ]pop
    obj shortDesc oldmud:updateShortDescInRoomCache
