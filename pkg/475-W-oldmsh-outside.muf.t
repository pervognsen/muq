...
( - 475-W-oldmsh-outside.muf -- Set outside view for mudUser shell.	)
...
( defmethod:  cmdHelpCategory { 'cmdOutside }	)
(    {[                         'it        ]}	)
(    [ "" |                              	)
( ;                                      	)
...
defmethod:  cmdHelp1 { 'cmdOutside }
    {[                   'it         ]}

    [ "Set outside description on <object>." |
;
...
defmethod:  cmdHelpN { 'cmdOutside }
    {[                   'it         ]}

    [
"Set outside description on given object: @outside <myname>=<myDescription>"
    |
;
...
defmethod: cmdDo { 'cmdOutside 't  't     }
        |shift -> it
        |shift -> av
        |shift -> cmdName
	'=' charInt |position -> pos
	pos not if "Syntax is '@outside <myname>=<myDescription>" errcho ]pop [ | return fi
    ...
    [ obj "out" "plain" desc | oldmud:doSetView ]pop
