...
( - 475-W-oldmsh-restart.muf -- Command for mudUser shell package.	)
...
( defmethod:  cmdHelpCategory { 'cmdRestart }	)
(    {[                         'it        ]}	)
(    [ "" |                              	)
( ;                                      	)
...
defmethod:  cmdHelp1 { 'cmdRestart }
    {[                   'it         ]}

    [ "Restart avatar daemon." |
;
...
defmethod:  cmdHelpN { 'cmdRestart }
    {[                   'it         ]}

    [
"Kills the currently running avatar daemon, if any,
and starts up another one."
    |
;
...
defmethod: cmdDo { 'cmdRestart 't  't     }
    {[              'it          'av 'name ]}

    "oldmsh" forkJob not if
	[   av.daemonShell		( function to ]exec 	)
	    av.io			( io queues to use	)
	    :home     av		( ]exec args		)
	| ]execUserDaemon
    fi
