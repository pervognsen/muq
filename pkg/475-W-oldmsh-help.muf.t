...
( - 475-W-oldmsh-help.muf -- Command for mudUser shell package.	)
...
( defmethod:  cmdHelpCategory { 'cmdHelp }	)
(    {[                         'it     ]}	)
(    [ "" |                              	)
( ;                                      	)
...
defmethod:  cmdHelpN { 'cmdHelp }
    {[                 'it      ]}

    [
"Print a quick summary of mudshell commands.
For now, reading the source is the best detailed reference.
The Muq docs may also sometimes be helpful."
    |
;
...
defmethod: cmdDo { 'cmdHelp 't  't     }
    ...
    "\n" ,
    "-------< Quick help for Muq oldmud shell >-------\n" ,
    opn keys[
	nil -> last
	|for i do{
	    opi[i] -> this
	    this last != if	( Ignore command abbreviations.		)
		[ "%15s -- " opn[i] | ]print , 
		[ this | cmdHelp1 ]shift , "\n" ,
	    fi
	    this -> last
