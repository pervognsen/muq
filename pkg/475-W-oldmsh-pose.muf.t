...
( - 475-W-oldmsh-pose.muf -- Do player 'pose'.				)
...
( Created:      97Oct26, from oldmshSay.				)
( Modified:								)
( Language:     MUF							)
( Package:      N/A							)
( Status:       							)
...
( defmethod:  cmdHelpCategory { 'cmdPose }	)
(    {[                         'it     ]}	)
(    [ "" |                              	)
( ;                                      	)
...
defmethod:  cmdHelp1 { 'cmdPose }
    {[                   'it      ]}

    [ "Pose <something> to others in current room." |
;
...
defmethod:  cmdHelpN { 'cmdPose }
    {[                   'it      ]}

    [
"Pose something to others in current room: pose <text>
Supported abbreviations are \"po\" \"p\" \";\" \":\" and \".\"."
    |
;
...
defmethod: cmdDo { 'cmdPose 't  't     }
	...
	av.name -> avName
	avName length 1- -> maxIdx

	' ' |unshift
	for i from maxIdx downto 0 do{   avName[i] |unshift   }
	...
	( Now restore message format for other users: )
	++ maxIdx
	for i from maxIdx downto 0 do{   |shiftp   }
