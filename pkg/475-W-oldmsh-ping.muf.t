...
( - 475-W-oldmsh-ping.muf -- Command for mudUser shell package.	)
...
( Created:      97Oct19, from 475-W-oldmsh-ping-self.t			)
( Modified:								)
( Language:     MUF							)
( Package:      N/A							)
( Status:       							)
...
( defmethod:  cmdHelpCategory { 'cmdPing }	)
(    {[                         'it     ]}	)
(    [ "" |                              	)
( ;                                      	)
...
defmethod:  cmdHelp1 { 'cmdPing }
    {[                   'it      ]}

    [ "See if <object> is alive." |
;
...
defmethod:  cmdHelpN { 'cmdPing }
    {[                   'it      ]}

    [
"Bounces a ping packet off the named object's daemon.
You should get a 'Ping succeeded!' message back if it is up.
Special cases:
   @ping me      -- Ping your own avatar daemon.
   @ping here    -- Ping current room.
   @ping home    -- Ping home room.
   @ping isle    -- Ping home isle."
    |
;
...
defmethod: cmdDo { 'cmdPing 't  't     }
    ...
    [   :op 'oldmud:REQ_PING
	    ...
	    ( Send success report to user shell: )
	    "* Ping succeeded *" echo
