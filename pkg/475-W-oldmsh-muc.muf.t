...
( - 475-W-oldmsh-muc.muf -- MUC command for mudUser shell package.	)
...
( Created:      99Oct21, from 475-W-oldmsh-muf.muf.t			)
( Modified:								)
( Language:     MUF							)
( Package:      N/A							)
( Status:       							)
...
( defmethod:  cmdHelpCategory { 'cmdMuc }	)
(    {[                         'it    ]}	)
(    [ "" |                              	)
( ;                                      	)
...
defmethod:  cmdHelpN { 'cmdMuc }
    {[                   'it     ]}

    [
"Enter the mucshell, which lets you interactively evaluate commands
in the MUC (Multi User C) programming language."
    |
;
...
: runMucShell { -> ! } muc:shell ;

defmethod: cmdDo { 'cmdMuc 't  't }
    {[             'it     'av 'name ]}

    "Entering MUC shell.  Your avatar is accessible as  @.task.taskState\n" ,
