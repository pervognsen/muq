...
( - 475-W-oldmsh-boot.muf -- Command for mudUser shell package.		)
...
( Created:      98Sep11, from 475-W-oldmsh-eject.t			)
( Modified:								)
( Language:     MUF							)
( Package:      N/A							)
( Status:       							)
...
( defmethod:  cmdHelpCategory { 'cmdBoot }	)
(    {[                         'it     ]}	)
(    [ "" |                              	)
( ;                                      	)
...
defmethod:  cmdHelpN { 'cmdBoot }
    {[                 'it     ]}

    [
"@boot <name> will kick <name> off your Muq server.
<name> must be logged in on your server (not visiting).
You must outrank <name>.  You may also want to
@ban <name>, to keep her/him from returning."
    |
;
...
(    {[            'it      'av 'name ]} )
    ...
    "@boot isn't implemented yet" errcho
