...
( - 475-W-oldmsh-tunnel.muf -- Command for mudUser shell package.	)
...
( Created:      98Sep12, from 475-W-oldmsh-dig.t			)
( Modified:								)
( Language:     MUF							)
( Package:      N/A							)
( Status:       							)
...
( defmethod:  cmdHelpCategory { 'cmdTunnel }	)
(    {[                         'it       ]}	)
(    [ "" |                              	)
( ;                                      	)
...
defmethod:  cmdHelp1 { 'cmdTunnel }
    {[                 'it       ]}

    [ "Dig tunnel connecting two existing rooms." |
;
...
defmethod:  cmdHelpN { 'cmdTunnel }
    {[                   'it     ]}

    [
"Connect two existing rooms: @tunnel <hereExitName>=<backExitName>=<roomName>."
    |
;
...
(    {[             'it       'av 'name ]} )
	...
	pos not if "Syntax is '@tunnel <exitName>=<backExitName>=<roomName>" errcho ]pop [ | return fi
	...
	( If room is remote, we know we don't own it without )
        ...
        here remote? if
	    "Can't @tunnel unless room is dig-ok or you own it" errcho
	    [ | return
        fi    
	...
	:am "cmdTunnel/REQ_CANONICAL_ROOM_NAME"
