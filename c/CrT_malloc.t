...
#define CRT_DEBUG_ALSO TRUE	/* Default to reasonable val if s/he didn't.*/
#endif
...
#ifndef CRT_MAGIC
#define CRT_MAGIC ((unsigned char)231)
#endif
...
#define CRT_FREE_MAGIC ((unsigned char)~CRT_MAGIC)
#endif
#ifndef CRT_PAD_BYTES
#define CRT_PAD_BYTES 8
#endif
...
#if CRT_DEBUG_ALSO
    unsigned char           top[CRT_PAD_BYTES];
...
#if CRT_DEBUG_ALSO
#define CRT_OVERHEAD_BYTES (sizeof(A_Header) +CRT_PAD_BYTES)
#else
...
check_block(
    ...
    int  i;
    for (i = CRT_PAD_BYTES; i --> 0; ) {
        if (m->top[i]  != CRT_MAGIC)   crash("Top overwritten",    m, file, line );
        if (m->end[i]  != CRT_MAGIC)   crash("Bottom overwritten", m, file, line ); }
...
CrT_malloc(
    ...
    m->end  = &((char*)m)[ size + (CRT_OVERHEAD_BYTES-CRT_PAD_BYTES) ];

    /* Write a sacred value at top+bottom of block: */
    {   int i;
        for (i = CRT_PAD_BYTES; i --> 0; ) {
            m->top[i] = CRT_MAGIC;
            m->end[i] = CRT_MAGIC;
