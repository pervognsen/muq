/*--   oglg.c -- Hackish interface to GLUT for Muq.			*/
...
/************************************************************************/
/*     glut_eval.c stuff -- >>> GLUT VERSION 3.7 <<<			*/
/************************************************************************/

/* {{{ blurb --								*/
/* The following code is adapted from glut_event.c.  We need to do	*/
/* this because GLUT is unfortunately designed to take over the		*/
/* program counter via glutMainLoop(), and to bypass this we need	*/
/* to be able to invoke processWindowWorkList() and			*/
/* processEventsAndTimeouts() directly: unfortunately, the stock	*/
/* glut_event.c not only declares these static, but has them access	*/
/* static variables to boot, shared with the public functions in that	*/
/* file. :(  So what we do here is to include all the static variables	*/
/* and functions from glut_event.c -- in principle, this could		*/
/* cause problems due to sharing of static variables between		*/
/* public and private functions &tc, but a careful scan of the glut3.6	*/
/* code reveals no such issues.						*/
/*								        */
/* NB: I've deliberately avoided cleaning up the formatting of this     */
/* code in order to facilitate merging in the changes from future       */
/* releases of GLUT (3.8...) via compare-windows in emacs.		*/
/*								        */
/* Notes to myself on GLUT internals:				        */
/*								        */
/* o GLUT_INT defines the following thirteen types of work bits:        */
/*    MAP							        */
/*    EVENT_MASK						        */
/*    REDISPLAY							        */
/*    CONFIGURE							        */
...
/*    OVERLAY_REPAIR						        */
/*								        */
/* Window worklist overview:					        */
/* o  __glutWindowWorklist heads a singly-linked queue		        */
/*   threaded via window->prevWorkWin pointers in window structs.	*/
/* o window->workMask is a bitmask of work for a given window.	        */
/*   This mask is zero on windows not on the worklist.		        */
/* o 'beforeEnd' is always supposed to point to the last	        */
/*   window on the worklist: It is used when appending.		        */
/*								        */
/* glutInit() overview:							*/
/*  cmdline arg parsing and deletion, including -geometry	        */
/*  __glutDisplay set to active X display. width &tc also noted.        */
/*								        */
/* glutCreateWindow() overview:						*/
/* o Creates a new X window.					        */
/* o Creates an internal glut window record for it.		        */
/* o Adds the latter to __glutWindowWorkList with MAP workbit set.      */
/*								        */
/* glutMainLoop() overview:					        */
/* o Checks that glutInit was called by checking _glutDisplay	        */
/* o Checks that at a window has been create via __glutWindowList       */
/* o Completes all work on one window before doing any on the		*/
/*   next, even if this requires multiple calls per window.		*/
/* o Enters an infinite loop in which the main two calls are:	        */
/*   o  processWindowWorkList()					        */
/*   o  processEventsAndTimeouts()				        */
/*								        */
/*								        */
/*								        */
/* processEventsAndTimeouts overview:				        */
/* o This does all the actual X event queue reading.		        */
/* o Loops until 'XPending()' is false, processing events.	        */
/* o Reads X input queue using interruptibleXNextEvent();		*/
/* o Event types handled are:					        */
/*   o  MappingNotify:    Internal X keyboard mapping update.	        */
/*   o  ConfigureNotify:					        */
/*	    Tells X to do the reshape;				        */
/*	    Calls user reshape function;				*/
/*	    Queues REPAIR_WORK on window.				*/
/*   o  Expose:								*/
/*	    Collapses Expose sequences to single Expose events.		*/
/*	    Queues REPAIR_WORK on window.				*/
/*   o  ButtonPress, ButtonRelease:					*/
/*	    Calls user 'mouse()' (or menu if appropriate).		*/
/*   o  MotionNotify:							*/
/*	    Calls user 'motion' or 'passivemotion' callback.		*/
/*   o  KeyPress, KeyRelease:						*/
/*	    Calls user keyboard OR keyboardUp callback,			*/
/*	    or user special() or specialUp() for cursor keys &tc.	*/
/*   o  EnterNotify, LeaveNotify:					*/
/*	    Call enter() and passiveMotion() callbacks as appropriate.	*/
/*   o  UnmapNotify:							*/
/*	    Used to update internal 'hidden' flag for window.		*/
/*   o  VisbilityNotify:						*/
/*	    Used to update internal visibility info.			*/
/*	    User windowStatus() callback is called.			*/
/*   o  ClientMessage:							*/
/*	    MAY TRIGGER AN exit(0);  :(					*/
/*   o  DestroyNotify:							*/
/*	    Deletes window record from StaleWindowList			*/
/* o Remaining event types are ignored unless handled			*/
/*   by registered extension event parsers.				*/
/*									*/
/*									*/
/*									*/
/*									*/
/* processWindowWorkList(win) overview:				        */
/*								        */
/* o Processes exactly one window from worklist (the first).	        */
/*								        */
/* o Return value is either that window (if work remains for it,        */
/*   say due to a callback scheduling some) or else the next	        */
/*   one on the worklist.					        */
/*								        */
/* o Is called only directly from glutMainLoop: There are no	        */
/*   direct or indirect recursive calls and no calls from elsewhere.    */
/*								        */
/* o Contains no major loops.					        */
/*								        */
/* o If win is last in worklist, sets 'beforeEnd' to point to it.       */
/*   This is a fairness trick so that any windows queued for	        */
/*   processing by callbacks during the course of the 		        */
/*   processWindowWorkList() call get processed -after-		        */
/*   any currently pending windows.				        */
/*								        */
/* o GLUT_EVENT_MASK_WORK: Change set of requested X events types.	*/
/*								        */
/* o GLUT_DEVICE_MASK_WORK: Something for trackballs &tc		*/
/*								        */
/* o GLUT_CONFIGURE_WORK: Do needed position, size and depth changes.	*/
/*								        */
/* o GLUT_COLORMAP_WORK: Establish colormap.			        */
/*								        */
/* o GLUT_MAP_WORK: Map/Unmap/Iconify/Fullscreen window.        	*/
/*								        */
/* o GLUT_REDISPLAY_WORK|GLUT_REPAIR_WORK: Invoke user display() fn.	*/
/*								        */
/* o GLUT_OVERLAY_REDISPLAY_WORK | GLUT_OVERLAY_REPAIR_WORK:		*/
/*   Invoke user overlay->display() fn.					*/
/*								        */
/* o GLUT_FINISH_WORK: glFinish() all windows that were	selected	*/
/*   as the current window (__glutSetWindow()) at some point.	        */
/*								        */
/* o GLUT_DEBUG_WORK: Calls glutReportErrors() if set.			*/
/*								        */
/*								        */
...
/************************************************************************
  ...
  "Consider emigrating.  With the exception of the U.S., every civilized
   nation provides some measure of income and health protection for its
   elderly."
          -- New York Times article on caring for elderly parents.
...
/* JSP: /usr/include/sys/param.h has a #define of 'MIN' */
/* which conflicts with that in Glutint37 -- when will  */
/* people learn namespace cleanliness?! We #undef it to */
/* avoid distracting compile warnings:                  */
#undef MIN

/***********************************************************/
/* (JSP:) This is an awful hack:                           */
/* X  and OpenGL have conflicting 'Cursor' types           */
/* /usr/include/X11/X.h:86: conflicting types for `Cursor' */
/* /usr/include/gl.h:993: previous declaration of `Cursor' */
/* The folowing #define lets us compile in spite of this:  */
/***********************************************************/
#define Cursor GLCursor
#include "../h/Glutint37.h"
#undef Cursor

/*static GLUTtimer *freeTimerList = NULL;*/
/*static int mappedMenuButton;*/

extern /*jsp*/ GLUTidleCB __glutIdleFunc;
extern /*jsp*/ GLUTtimer *__glutTimerList;

#ifdef SUPPORT_FORTRAN
/** GLUTtimer *__glutNewTimer; jsp*/
#endif
/** GLUTwindow *__glutWindowWorkList = NULL; jsp*/
/** void (*__glutUpdateInputDeviceMaskFunc) (GLUTwindow *); jsp*/
/** Atom __glutMotifHints = None; jsp*/
/* Modifier mask of ~0 implies not in core input callback. */
/** unsigned int __glutModifierMask = (unsigned int) ~0; jsp*/
/** int __glutWindowDamaged = 0; jsp*/

extern void (*__glutUpdateInputDeviceMaskFunc) (GLUTwindow *);
#if !defined(_WIN32)
extern void (*__glutMenuItemEnterOrLeave)(GLUTmenuItem * item, int num, int type) /*= NULL*/;
...
extern Atom __glutMotifHints /*= None*/;
/* Modifier mask of ~0 implies not in core input callback. */
extern unsigned int __glutModifierMask /*= (unsigned int) ~0*/;
extern int __glutWindowDamaged /*= 0*/;



#ifdef WE_WILL_GET_THIS_FROM_GLUT_PROPER /**jsp*/
void APIENTRY
